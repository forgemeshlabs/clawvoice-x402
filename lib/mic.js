...
async function ensureMicVenv({ dryRun, yes }) {
  ...
  if (dryRun) {
    ...
    for (const [cmd, args] of steps) console.log(`Would run: ${[cmd, ...args].join(" ")}`);
    return python;
  }

  if (!commandExists("python3")) {
    console.error("python3 not found. Install Python 3 first, then rerun: x402-agent-voice install-mic");
    process.exit(1);
  }
